...
export function loadFullStore() {
    const db = getDb();
    const projects = db.prepare("SELECT * FROM projects").all();
    const members = db.prepare("SELECT * FROM members").all();
    const sprints = db.prepare("SELECT * FROM sprints").all();
    const stories = db.prepare("SELECT * FROM stories").all();
    const sprintStories = db.prepare("SELECT * FROM sprint_stories ORDER BY position").all();
    const membersByProject = new Map();
    for (const m of members) {
        const member = { id: m.id, name: m.name, role: m.role, avatar: m.avatar || undefined };
    ...
    for (const s of stories) {
        ...
        if (!storiesByProject.has(s.project_id))
            storiesByProject.set(s.project_id, []);
    ...
    for (const s of sprints) {
        ...
        if (!sprintsByProject.has(s.project_id))
            sprintsByProject.set(s.project_id, []);
    ...
    const activeProjectId = db.prepare("SELECT value FROM config WHERE key = 'active_project_id'").get();
    return {
...
export function saveFullStore(store) {
    const db = getDb();
